...
│  ├─ shared/            # PERMISSIONS 상수, Zod validation, audit-log helper, 공통 타입
...
├─ docker/               # secrets/ 만 잔존 (turbopack 마이그레이션 후 compose/Dockerfile 삭제됨)
├─ docs/                 # plan/, spec/, adr/, _archive/
└─ scripts/              # check-llm-models, wiki-check, audit-rsc, eval-budget-test
```
...
| LLM | OpenAI API 직결 (Anthropic SDK 옵션) — `packages/ai/agent/**` | budget 게이트는 `packages/ai/budget.ts`. 과거 docker cli-proxy 게이트웨이는 turbopack 마이그레이션에서 삭제됨 |
| 임베딩 | **완전 제거됨** (Phase D+E, 2026-04) — `packages/ai/embed.ts` / `router.ts` 모두 삭제 | Ask AI는 4-tool agent (wiki_grep/read/follow_link/graph_query) |
...
**중요:** 과거의 "6-lane 라우터 + page-first retrieval" 설계는 **Phase A–G(2026-04) 완료로 tool-use agent로 전환됨**. embedding RAG·벡터 유사도·`FEATURE_PAGE_FIRST_QUERY` 플래그는 모두 제거됨. 전환 이력은 git log (`git log --grep='harness-first'`) 참조 — plan 문서는 `_archive`로 이동됨.
...
`packages/ai/router.ts` (6-lane 라우터) / `retrieveRelevant*` / `generateEmbedding` / `embed_cache` 쓰기 경로 / HNSW 인덱스 / `FEATURE_PAGE_FIRST_QUERY` 플래그는 **모두 삭제 완료**. 신규 코드는 `packages/ai/agent/**`만 참조한다.
...
- `packages/shared/constants/permissions.ts`에 **PERMISSIONS 23종** 정의 — 변형 2-tier (모든 도메인 `{read, admin}` + `admin:all` 마스터). 도메인: knowledge, project(+ additional-dev 흡수), notice, maintenance(+ service-desk + month-report 흡수), infra, doc-num, faq, graph, user(+ contractor 흡수), schedule, sales, admin
- `ROLE_PERMISSIONS` 매핑: **4역할** (ADMIN, MANAGER, MEMBER, YEAREND). 한글 라벨: `ROLE_LABELS` (관리자/매니저/일반/연말정산)
  - ADMIN: 23 권한 전부
  - MANAGER: 21 권한 (`user:admin` + `admin:all` 제외)
  - MEMBER: 10 권한 (모든 도메인 read + `schedule:admin` — 본인 일정 owner check로 격리)
  - YEAREND: 0 권한 (외부 yearend 사이트 식별 라벨)
- owner check 패턴 (RBAC 밖 도메인 로직): knowledge / schedule 2도메인만 적용. `created_by === session.userId` OR `knowledge_page_owner`에 row 존재. `admin:all` 보유자는 우회
- 격리는 RBAC 권한 + `workspaceId` 일치만 사용. 행 단위 sensitivity 필터는 2026-05-12 폐기
...
# DB
# 스키마 변경: packages/db/schema/*.ts 수정 → 운영 DB에 ALTER/CREATE SQL 직접 적용
...
- **Next.js web**: `pnpm dev` → http://localhost:3010
- **PostgreSQL / MinIO**: 운영 원격 서버에 직접 연결 (`.env`의 `DATABASE_URL` / `MINIO_ENDPOINT`). 로컬 docker 인프라 없음.
...
| 권한 (23 상수) | 기존 재사용? 새 도메인이면 read/admin 추가? 4 역할 매핑? | `packages/shared/constants/permissions.ts`, `packages/auth/rbac.ts` |
...
| LLM 호출 | OpenAI 직결, `llm_call_log` 기록? budget 영향? | `packages/ai/agent/ask-agent.ts`, `packages/ai/budget.ts` |
...
```
 1. packages/db/schema/*.ts                 (스키마 — 변경 후 운영 DB에 SQL 직접 적용)
 2. packages/shared/validation/*.ts         (Zod 입출력 스키마)
 3. packages/shared/constants/permissions.ts (권한 상수 + ROLE_PERMISSIONS 매핑)
 4. packages/auth/rbac.ts, session.ts       (권한/세션 헬퍼 — 필요 시만)
 5. packages/secret/*                        (SecretRef — 필요 시)
 6. packages/wiki-fs/src/**                 (디스크 I/O + git) ← stateful
 7. packages/wiki-agent/src/**              (LLM prompt/parser) ← stateless
 8. packages/ai/**                          (router, page-first, ask, tutor, 6 contexts)
 9. packages/search/**                      (FTS/trigram/precedent 독립)
10. apps/web/lib/**                         (웹 전용 쿼리/헬퍼)
11. apps/web/app/actions/**                 (도메인 횡단 server action)
12. apps/web/app/(app)/**/actions.ts        (도메인별 server action)
13. apps/web/app/api/**/route.ts            (REST endpoint — 필요 시)
14. apps/web/app/(app)/**/page.tsx          (Server Component)
15. apps/web/app/(app)/**/_components/*.tsx (Client Component)
16. apps/web/messages/ko.json               (i18n — 배치 처리)
17. apps/worker/src/jobs/ingest/*.ts        (ingest 4단계 — 필요 시)
18. apps/worker/src/jobs/*.ts + index.ts    (기타 워커 잡 + 스케줄 등록)
19. 테스트 파일 (*.test.ts, e2e/*.spec.ts, worker/eval/)
```
...
| 운영 DB에 `ALTER/CREATE` SQL 직접 적용 | 스키마 변경 시 | 스키마 동기화 |
...
- **LLM 호출 경로** — `packages/ai/agent/ask-agent.ts`가 OpenAI/Anthropic SDK 직접 호출. docker cli-proxy 게이트웨이와 `packages/ai/router.ts`는 turbopack 마이그레이션에서 모두 삭제됨
- **엔터프라이즈 스케일, 1주 스프린트** — 속도 우선, 품질 유지

## 페이지 표준 (PageShell — 모든 (app) 라우트)

모든 `apps/web/app/(app)/**/page.tsx`는 **PageShell** 또는 **PageShellFit**을 사용한다. 페이지마다 자체 wrapper(`<div className="space-y-6">`, `mx-auto max-w-...`, `height: calc(...)`, `overflow-hidden` 등)를 만들지 않는다. 좌우 padding · max-w · 위 chrome · 페이지 스크롤 정책은 **AppShellMain 전역 프레임 + PageShell이 단일 source**로 결정.

**컴포넌트** ([`apps/web/components/patterns/PageShell.tsx`](../../../apps/web/components/patterns/PageShell.tsx)):

| API | 모드 | 사용처 |
|-----|------|--------|
| `<PageShell title actions>{...}</PageShell>` | **자연 height** (default, safe) — wrapper의 `overflow-y-auto`가 페이지 스크롤 처리 | 공지·지식 본문·폼·자연 height 페이지 (대부분) |
| `<PageShellFit title actions>{...}</PageShellFit>` | **viewport-fit** — `h-full flex-col gap-3 overflow-hidden`. 페이지 자체 스크롤 X, 내부 위젯만 스크롤 | 대시보드 / admin 그리드 / 채팅 / 어떤 화면에서도 페이지 스크롤을 절대 원치 않는 곳 |

두 모드 모두 동일하게:
- PageHeader 30px h1 + actions slot 우측 정렬
- 자식들 사이 12px gap (`space-y-3` 또는 `gap-3`)
- 좌우 padding / max-w / 위 chrome은 AppShellMain 공통 (페이지에서 손대지 않음)

**규칙 (PR 시 spec-reviewer 검증):**
- 신규 page.tsx는 반드시 `<PageShell>` 또는 `<PageShellFit>` 사용. raw `<div className="space-y-6">` 같은 자체 wrapper 금지.
- 페이지에서 `mx-auto` / `max-w-[...]` / `px-*` / `py-*` / `style={{height: 'calc(100vh - ...)'}}` 사용 금지. AppShellMain 전역이 처리.
- PageHeader import도 직접 하지 않음 (PageShell이 내부에서 처리).
- 자연 height vs viewport-fit 결정 모호하면 `PageShell` (default, safe). 컨텐츠가 길어지면 wrapper 스크롤이 처리. 페이지 자체 스크롤 원치 않는 경우만 `PageShellFit`.

**Phase 1 적용 (2026-05-16):** admin/menus, dashboard, holidays, infra. 나머지 ~74 page.tsx는 점진 sweep 예정. 기존 raw wrapper 페이지는 동작하지만 일관성 부족 — PR에서 손대는 page는 PageShell로 마이그레이션.

## 그리드 표준 화면 (대량 데이터 마스터)

다량의 행 데이터를 표시·편집하는 모든 화면은 **DataGrid baseline**을 따른다. 시각·동작 표준은 `admin/companies` 화면이 reference implementation. 행 ≥ 20건이면 무조건 그리드 채택 — 카드형/모달폼 X.

**그리드 작업 시 반드시 [`references/grid-standard.md`](references/grid-standard.md)를 Read한다.** 거기에 다음이 정의됨:
1. baseline 컴포넌트 위치 (공유 인프라 + 도메인 wrapping 규칙)
2. 필수 기능 7+1+1 (인라인 편집, dirty tracking, 툴바 3버튼, 필터 row, 서버 페이징, confirm dialog, batch save, Excel export, **조회 클릭 시 자동 reset** — `GridSearchForm.onResetGrid` required prop)
3. 디자인 토큰 (행 32px, 셀 패딩, 폰트, 색상 변수)
4. 버튼 표준화 (i18n 라벨, variant, 상태별 텍스트)
5. 컬럼 컨벤션 (PK → 본문 → audit readonly)
6. server action 컨벤션 (`list{Domain}`/`save{Domain}` 두 함수 + 트랜잭션 + audit_log)
7. 금지 패턴 9건 (TanStack Table 신규 / 카드형 / 모달 폼 / 무한 스크롤 / raw 색상 등)
8. 신규 그리드 PR 체크리스트 11항 (spec-reviewer 검증용)
9. 참고 (reference impl + baseline 사용처 11곳)
...
**핵심 규칙:**
- 데이터 SoT: `menu_item` 테이블 (workspace 스코프, `code` unique). 옛 `apps/web/lib/routes.ts` 하드코딩 라우트 카탈로그는 turbopack 마이그레이션에서 삭제됨.
...
3. 시드 SQL을 운영 DB에 직접 적용 (멱등 — `onConflictDoUpdate` on `(workspace_id, code)` 전제).
...
### 메뉴 그룹·sortOrder·권한 매트릭스 (현재 운영 DB 상태, 2026-05-12 기준 77 entry)

| sortOrder 범위 | 그룹 | 노드 수 |
|---|---|---|
| 10~140 | NAV (탑레벨 사이드바) | 14 |
| 81~93, 111, 131~132 | NAV 자식 (knowledge/projects/infra/contractors) | 12 |
| 145~160 | 운영(스케줄·유지보수·리포트) | 4 |
| 200~228 | SALES_GROUP + 자식 | 28 |
| 300~317 | ADMIN_GROUP + 자식 | 19 |

**권한 매핑 정책:** 각 메뉴 1+개 권한과 N:M 매핑(`menu_permission`). UNION-match 가시성. 그룹 헤더(parent)는 자식 표시 여부로 cascade. **2026-05-16 RBAC simplification 후 매트릭스:**

| 도메인 그룹 | 권한 | ADMIN | MANAGER | MEMBER | YEAREND |
|---|---|:-:|:-:|:-:|:-:|
| Knowledge 7 (FAQ/용어집/HR/온보딩/도구/퀴즈×2) | `knowledge:read` | ✓ | ✓ | ✓ | ✗ |
| Projects 자식 3 (비콘/이력/모듈, 추가개발 흡수) | `project:read` | ✓ | ✓ | ✓ | ✗ |
| Infra 런북 | `infra:read` | ✓ | ✓ | ✓ | ✗ |
| 스케줄 | `schedule:read` (+ admin = 본인 일정) | ✓ | ✓ | ✓ | ✗ |
| 유지보수 배정 (service-desk + month-report 흡수) | `maintenance:read` | ✓ | ✓ | ✓ | ✗ |
| Contractors (외부 인력 — user로 흡수) | `user:read` | ✓ | ✓ | ✗ | ✗ |
| Sales 그룹 (대시보드/계약/영업기회/활동/차트 등) | `sales:read`/`sales:admin` | ✓ | ✓ | ✗ | ✗ |
| Admin 그룹 (사용자·역할·조직·감사·회사·문서번호·FAQ·인프라·임포트 등) | `admin:all` | ✓ | ✗ | ✗ | ✗ |

**라우트 정합성 정책 (2026-05-12 확정):**
- 소스 `apps/web/app/(app)/**/page.tsx` 라우트 1개당 menu_item 1개(또는 의도적 제외) 원칙.
- 의도적 제외: `/profile` (Topbar dropdown), 모든 `[id]` / `[...path]` / `[id]/edit` / `new` 동적·진입·편집 라우트.
- `route_path` 중복 금지 — `SALES_COMPANIES` 가 `/admin/companies` 로 mis-pointing 됐던 케이스 정정 완료(→ `/sales/companies`).
- 신규 도메인 라우트 추가 시 menu_item + menu_permission seed 동시 작성.

**최근 일괄 등록 (2026-05-12, 운영 DB 직접 적용):**
- 42 menu_item INSERT (Knowledge 7 / Projects 3 / Infra 1 / 운영 5 / Sales 22 / Admin 4) + 42 menu_permission.
- 4 ADMIN_* 권한 tighten: `doc-num:read` / `faq:read` / `infra:admin` / `infra:write` → 일괄 `admin:all` (ADMIN-only 가시성, ADMIN_GROUP 형제 ADMIN_USERS / ADMIN_AUDIT 패턴과 일치).
- 산출 SQL은 `.local/sql/` 외부 보존 안 함 (멱등 INSERT … ON CONFLICT DO NOTHING + DELETE+INSERT 정정).
