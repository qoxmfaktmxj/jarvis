...
`requirePermission(PERMISSIONS.X)`은 `packages/auth/session.ts`에서 export. 세션 + 권한을 한 번에 해결. (sensitivity 헬퍼 14개는 2026-05-12 일괄 제거됨 — §4 정책 참조.)
...
- [x] `workspaceId` + `userId` 이중 필터 (행 단위 sensitivity 필터 없음 — §4)
...
- 격리는 workspaceId + userId 이중 필터 (§4 정책)
...
도메인 조회의 격리는 RBAC + workspaceId만 사용한다 (§4 정책).
...
폐기된 sensitivity 헬퍼(§4) 사용 발견 시 즉시 반려.
