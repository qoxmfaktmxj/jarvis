...
Jarvis의 DB 레이어는 `packages/db/schema/*.ts`에 도메인별로 분리되어 있고(**54 파일 / ~70 테이블** — 2026-05 기준 ETL/sales/wiki-projection 확장으로 증가), 변경은 Drizzle → Zod validation → 권한 체크 → sensitivity 필터까지 연쇄된다. 이 스킬은 그 체인을 한 번에 올바르게 다루기 위한 참조다.
