...
| DB 스키마·마이그레이션·RBAC·Zod·server action·트랜잭션 | `jarvis-db-patterns` |
...
- DB 스키마 / Validation / 권한(47상수·5역할) / 세션 vs 권한 모델 / workspaceId 격리
- Ask AI 4-tool agent / Wiki-fs Karpathy 경계 / 검색 선택(pg-search vs precedent-search)
...
**계획 승인 조건:** 아키텍처 결정 / 새 PERMISSION 생성 / 파괴적 마이그레이션이 포함되면 사용자 승인 후 Phase 4.
...
| spec-reviewer | `jarvis-db-patterns`의 "경계면 교차 비교 체크리스트"(shape·권한·nullable·마이그레이션·i18n 교차 비교). `jarvis-i18n`의 "경계면 검증" 섹션. wiki 변경이면 `jarvis-wiki-feature`의 "Karpathy 원칙 4가지" + "자주 하는 실수". |
...
- [`jarvis-db-patterns`](../jarvis-db-patterns/SKILL.md) — 스키마 54 파일·47 권한·5 역할·server action·경계면 교차 비교
