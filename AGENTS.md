...
**상태:** 디자인 시스템 v4 적용됨 (2026-05-16). `--bg-page` warm-50 + `--bg-surface` `#fff` swap, `--brand-primary` 단일 SoT + `color-mix(in oklab)` 파생, 5테마 picker(`data-theme-color`). 새 UI는 `--brand-primary*` 토큰만 사용 — hex 직접 하드코딩 금지(테마 cascade 끊김). 카드는 `bg-[--bg-surface]`, 페이지는 `bg-[--bg-page]`. 자세한 정책은 [`docs/design-system.md`](docs/design-system.md) v4.
