...
**상태:** 디자인 시스템 v5 적용됨 (2026-05-16). 통합 bg 토큰 — 페이지 + 카드 + chrome 모두 `--bg-page` (#fff). `--bg-surface` (warm-50)는 미세 tint 전용 (code / DataGrid thead / tab hover). `--brand-primary` 단일 SoT + `color-mix(in oklab)` 파생 + 5테마 picker (`data-theme-color`). 새 UI는 `--brand-primary*` 토큰만 사용 — hex 직접 하드코딩 금지. 카드는 `bg-[--bg-page] border border-[--border-default] shadow-[var(--shadow-soft)]` (페이지와 같은 색, border+shadow로만 구분). 자세한 정책은 [`docs/design-system.md`](docs/design-system.md) v5.
...
| [`.claude/skills/jarvis-db-patterns/SKILL.md`](.claude/skills/jarvis-db-patterns/SKILL.md) | 54 스키마 파일 · 23 권한 · 4 역할 · Zod · server action 두 패턴(RBAC / 세션) · **경계면 교차 비교 체크리스트**(shape/권한/nullable/마이그레이션/i18n) | DB·권한·서버 액션 작업 |
...
- 상수: `packages/shared/constants/permissions.ts` (`PERMISSIONS` 23종). 역할 매핑: 같은 파일 `ROLE_PERMISSIONS` (4역할 — ADMIN/MANAGER/MEMBER/YEAREND). 한글 라벨: `ROLE_LABELS` (관리자/매니저/일반/연말정산).
...
| 2026-05-16 | **RBAC 단순화 반영 — 47권한·5역할 → 23권한·4역할.** §핵심 규칙 §권한 (RBAC) — `PERMISSIONS 47종` → `23종`, `ROLE_PERMISSIONS (5역할)` → `(4역할 — ADMIN/MANAGER/MEMBER/YEAREND)` + 한글 라벨 `ROLE_LABELS`(관리자/매니저/일반/연말정산) 추가. 도메인 스킬 표(jarvis-db-patterns 줄) — 권한 47→23, 역할 5→4. 변형 2-tier 패턴(모든 도메인 `{read, admin}` + `admin:all` 마스터). owner check 패턴 도입(knowledge/schedule 2도메인만 — `created_by` OR `knowledge_page_owner`, `ADMIN_ALL` 우회). `/admin/roles` 신규 페이지(master/detail role↔permission 매핑 UI). Ask AI bypass 플래그 `ASK_AI_RBAC_BYPASS=false`(기본, prod 안전). 본문 변경은 CLAUDE.md/README.md §18 entry + 5 하네스 스킬(jarvis-architecture/db-patterns/feature/wiki-feature/i18n) drift 갱신과 정합. | 사용자 요구 "UI에 47권한 다 보여서 비대 — 단순화". 변형 2-tier가 표준 RBAC 패턴(AWS IAM ReadOnlyAccess + AdministratorAccess 동일)임을 확인. codex review P1+P2 8건 fix 완료. 자세한 이력은 `CLAUDE.md` 2026-05-16 entry. |
