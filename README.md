...
디스크 `wiki/` 디렉터리는 **서버 내부 전용 스토리지**입니다. 사용자가 Obsidian이나 VS Code로 직접 열지 않습니다. 접근은 전부 Next.js 웹 UI + HTTP API 경유이며, 권한 게이팅(RBAC + workspaceId)·감사 로그가 전부 이 경로에서만 동작합니다.
...
| **ORM** | Drizzle ORM | schema는 `packages/db/schema/**` TS가 SSoT |
...
| 테이블 | 역할 |
|--------|------|
| `wiki_page_index` | 파일시스템 미러 — path, title, frontmatter, workspaceId, updatedAt, gitSha |
...
외부 PostgreSQL / MinIO에 직접 연결 — 로컬 인프라 불필요. `.env`의 `DATABASE_URL`, `MINIO_*` 등을 운영 또는 사내 개발 서버로 설정한다.
...
pnpm lint
pnpm type-check
node scripts/check-llm-models.mjs     # 허용된 LLM 모델 lint
```
...
### 11.2 인가 (RBAC + workspaceId)

파일시스템에는 RBAC가 없으므로, **frontmatter + 서버 미들웨어 게이트**로 풀어냅니다. 행 단위 sensitivity 필터는 사용하지 않으며, 격리는 **workspaceId 멀티테넌트 + RBAC 권한 게이트** 두 축으로만 합니다(2026-05-12 단순화).
...
- 접근 제어는 **RBAC 권한(`KNOWLEDGE_READ` 등) + 세션 `workspaceId` 일치**로만 결정. 페이지별 sensitivity/`requiredPermission` 분기는 폐기.
...
**자동화:** `.claude/settings.json` PostToolUse 훅이 LLM 모델 사용(`scripts/check-llm-models.mjs`)을 advisory로 감지. CI/pre-commit은 blocking 모드로 재사용.
...
| RBAC | 유지 — sensitivity row-level filter는 2026-05-12 폐기, workspaceId + RBAC 단일화 |
...
이 공개 리포는 외부 사용자가 아키텍처를 학습하고 자체 환경을 구성할 수 있도록 설계되었습니다. 사내 사용자는 zip 채널로 추가 자료(시드 SQL·legacy 이관 스크립트)를 별도로 받습니다.

### 17.1 외부 사용자 (이 공개 리포만 가진 경우)

- DB 스키마는 `packages/db/schema/**`가 진실원천(SSoT). 테이블/컬럼 정의는 이 TS 파일이 기준.
- 자체 환경에서 스키마를 DB에 적용하려면 TS 파일을 기반으로 SQL을 직접 생성해 운영 DB에 적용.
- 시드 데이터는 직접 작성 또는 dummy 데이터 사용.
- 영업관리·legacy Oracle 이관 스크립트는 사내 한정이므로 이 리포에 없음.

### 17.2 사내 사용자 (zip 채널로 추가 자료를 받은 경우)

zip에는 다음 자료가 포함됩니다:

- `packages/db/seed/` — TS 시드 스크립트 (workspace·role·permission·menu·holiday)
- `.local/legacy-ssms/postgres/<NN>_<module>/*.sql` — ETL 출력 SQL (legacy Oracle → jarvis 변환 결과). 멱등 INSERT, deterministic UUID
- `.local/docs/` — 메타 전략 spec/plan + 작업 인수인계 문서

**DB 적용 절차:** zip 자료를 로컬에 복원 후, `.local/legacy-ssms/postgres/` 하위 SQL 파일을 운영 DB에 직접 psql로 적용합니다. 자동화 스크립트는 제거되었으므로 운영자가 파일을 확인하고 순서대로 적용합니다.

ETL 폴더 컨벤션(10/20/30 단위 prefix, 멱등 INSERT, deterministic UUID 등)은 사내 zip 채널의 `.local/docs/2026-05-04-legacy-etl-meta-strategy-design.md` §3-§4 참조.
...
| 2026-05-11 | **React/Next.js 보안 패치 (12 CVE).** `apps/web/package.json`: `next ^15.5.15 → ^15.5.16` (lock: 15.5.18), `react ^19.0.0 → ^19.2.6` (lock: 19.2.6), `react-dom ^19.0.0 → ^19.2.6` (lock: 19.2.6), `eslint-config-next ^15.5.15 → ^15.5.16`. React Team + Vercel이 2026-05 공동 공개한 보안 권고 — High 6건(RSC DoS, segment-prefetch middleware bypass, Cache Components connection exhaustion DoS, 동적 라우트 파라미터 미들웨어 우회, WebSocket SSRF, Pages Router i18n middleware bypass) + Moderate 4건(CSP nonce XSS, beforeInteractive XSS, Image Optimization DoS, RSC cache poisoning) + Low 2건. App Router only이라 Pages Router i18n CVE는 직접 미해당이나 패치 자체는 동일 버전에 포함. 검증: type-check/lint/build/audit:rsc 모두 통과(ERROR 0, WARN은 기존 baseline 동일). breaking change 없음 (patch level only). | WAF 차단 불가능한 High 6건(미들웨어 우회·SSRF) 포함 → 애플리케이션 코드 패치가 유일한 대응 수단. Cloudflare 권고(2026-05) 즉시 반영. |
| 2026-05-16 | **RBAC 단순화 — 47권한·5역할 → 23권한·4역할.** 변형 2-tier 패턴(모든 도메인 `{read, admin}` + `admin:all` 마스터). 폐기 24권한은 도메인 흡수(knowledge/project/notice CRUD→admin · additional-dev→project · service-desk+month-report→maintenance · contractor→user · audit:read+files:write→admin:all · sales:all→sales:admin · *:write→*:admin). 5역할 → 4역할: ADMIN/MANAGER 유지(name 한글 — 관리자/매니저), DEVELOPER+HR+VIEWER → MEMBER(일반), 신규 YEAREND(연말정산, 외부 yearend 사이트 식별 라벨, jarvis 권한 0). 한글 라벨 `ROLE_LABELS` 신규. owner check 패턴 도입(knowledge PUT/DELETE/review + schedule update/delete — `created_by` OR `knowledge_page_owner` 매칭, `ADMIN_ALL` 우회). `/admin/roles` 신규 페이지(master/detail 그리드, role↔permission 매핑 UI, 시스템 역할 권한 매핑 immutable). Ask AI bypass 플래그 `ASK_AI_RBAC_BYPASS=false`(기본, prod 안전 — true는 테스트 전용). 운영 DB 10단계 트랜잭션 마이그레이션 적용 완료(permissions=23, roles=4, role_permissions=54, orphan=0). codex review P1+P2 8건 fix 완료. | Cloudflare 권고 무관, 사용자 요구 "UI에 47권한 다 보여서 비대 — 단순화". 디스커션 7라운드 결정: ① 변형 2-tier ② 4역할 ③ owner check는 knowledge/schedule 2도메인만 ④ Ask AI bypass 테스트 단계 ⑤ `/admin/roles` UI 신설. read/admin이 표준 RBAC 패턴(AWS IAM ReadOnlyAccess + AdministratorAccess 동일)임을 확인. 영향: SoT 1 파일 + 호출처 39 파일 + 운영 DB 트랜잭션 + 신규 페이지 1 + i18n 36키 + e2e 4역할 갱신 + 5 하네스 스킬 + AGENTS.md + CONTRIBUTING.md + WIKI-AGENTS.md drift 갱신. |
| 2026-05-12 | **Sensitivity row-level filter 전면 폐기 — RBAC + workspaceId 단일화.** 8 테이블(`knowledge_page`/`knowledge_claim`/`wiki_page_index`/`notice`/`precedent_case`/`raw_source`/`graph_snapshot`/`project`/`infra_system`)의 `sensitivity` 컬럼 + `wiki_page_index.required_permission` 컬럼 + `notice_sensitivity` enum + `idx_infra_system_ws_sens` 인덱스 일괄 drop. auth helper 14개(`canAccessKnowledgeSensitivity`/`canAccessKnowledgeSensitivityByPermissions`/`buildLegacyKnowledgeSensitivitySqlFilter`/`buildLegacyKnowledgeSensitivitySqlFragment`/`buildWikiSensitivitySqlFragment`/`getAllowedWikiSensitivityValues`/`canResolveProjectSecrets`/`canAccessGraphSnapshotSensitivity`/`buildGraphSnapshotSensitivitySqlFragment`/`resolveAllowedWikiSensitivities`/`buildWikiSensitivitySqlFilter`/`legacyCanAccessSensitivity`/`canViewWikiPage`/`canViewInternalNotice`) 완전 제거. `packages/auth/wiki-acl.ts` 파일 + `apps/web/lib/server/wiki-sensitivity.ts` 파일 삭제. Ask AI 4-tool agent: `withSensitivityFilter` → `withWorkspaceRbacFilter`로 단순화(시그니처는 그대로 — workspaceId/userId/permissions 검증만 유지). UI sensitivity 입력/표시/배지(`<select>` 4option, `<Badge>{sensitivity}</Badge>`) 일괄 제거. A9 F1 notice `canViewInternal` fix revert · A8 B-P0-02 wiki ACL 게이팅 폐지 · Q1/Q2 자동 해소 · `jarvis-db-patterns §4 §9.3` 정책 폐기 · D5 `SecretRef` 권한 분기 제거(`PROJECT_READ` 보유 시 모두 노출). | `packages/db/schema/{knowledge,wiki-page-index,notice,case,file,graph,project,system}.ts`, `packages/auth/{rbac,wiki-acl,index}.ts`, `packages/ai/agent/tools/**`, `apps/web/lib/queries/{notices,projects,infra-system,knowledge,dashboard-wiki-*}.ts`, `apps/web/app/(app)/{knowledge,wiki,notices,projects,infra,sales/*}/*`, `apps/web/messages/ko.json`, `apps/web/lib/server/wiki-sensitivity.ts` 삭제, `packages/db/drizzle/<NNNN>_sensitivity_drop.sql`(gitignored, 로컬 DB 적용 완료) | 사용자 결정 "RBAC만 하고 화면 자체를 안 보여주는 방식". 5개 결정(D1=B/D2=B/D3=B/D4=A/D5)으로 멀티 workspace 격리가 row 단위 sensitivity와 동등한 보안 경계를 제공함을 확인 — row 단위 필터가 진짜 필요한 도메인 없음. helper drift 다수 + 모순 정책(Plan 5 vs §9.3) 일소. |
| 2026-05-16 | **디자인 시스템 v4 — Q5=B swap + brand-primary `color-mix` 파생 + 5테마 picker + 사이드바 active 시각 + 다크 동일 적용.** mockup 2회 검증 후 사용자 결정 11건 기반 단일 worktree. `apps/web/app/globals.css`: `--bg-page` `#fff → warm-50(#faf9f8)` + `--bg-surface` `warm-50 → #ffffff` swap(회색 띠 일소) · `--brand-primary-hover/bg/text`를 `color-mix(in oklab, --brand-primary 80%/8%, ...)`로 자동 파생 · prototype alias(`--panel`/`--bg`/`--accent*`) + shadcn bridge(`--color-primary`/`--color-accent`/`--color-accent-foreground`/`--color-ring`) 모두 SoT(`--brand-primary*`) 참조로 통일 → 테마 picker가 shadcn UI에도 cascade · `--border-focus` `var(--brand-primary)` 참조 · `data-theme-color={blue,indigo,teal,forest,graphite}` 10블록(light + dark) · Graphite 다크 `#171717 → #f5f5f5` 반전(다크 bg 충돌 방지). **Sunset `#dd5b00` 제외** — `--color-orange` Warn 상태색과 hex 완전 동일(의미 충돌, hex 보정 불가). Forest `#1f883d → #0f6e3a` 보정(Done chip과 명도 분리). `Sidebar.tsx`: expanded active `var(--line2)` warm pill → `var(--brand-primary-bg)` 8% 틴트 + `var(--brand-primary-text)` 글자 + 좌측 3px `var(--brand-primary)` indicator(rail와 패턴 통일). `uiPrefs.ts`: `useThemeColor`/`setThemeColor` + `THEME_COLOR_IDS as const` typed whitelist + `UI_PREFS_BOOTSTRAP` FOUC 방지 inline script 확장(`data-theme-color` 추가). `ThemeColorPicker.tsx`(신설): 5 swatch radio group(aria-checked/aria-label/키보드). `UserMenu.tsx`: "테마 설정 [준비 중]" disabled → 활성 submenu(aria-haspopup/expanded + ChevronDown rotation + dropdown 닫힘 시 picker reset useEffect). i18n: `Theme.picker.{title,aria}` + `Theme.colors.{blue,indigo,teal,forest,graphite}` + `Common.themeColor`/`themeColorRegion` (ko + en, 9×2 = 18 키). **TDD**: uiPrefs 6 vitest + ThemeColorPicker 5 vitest + theme-picker 5 playwright(2회 연속 5/5 PASS). 검증 게이트: type-check×2 + lint×2 + audit:rsc(ERROR 0) + build PASS. `docs/design-system.md` v3 → v4 갱신(swap + 5테마 picker 섹션 + 토큰 파생 모델). | `apps/web/app/globals.css`, `apps/web/components/layout/{Sidebar,UserMenu,uiPrefs,ThemeColorPicker,ThemeColorPicker.test,uiPrefs.test}.tsx/ts`, `apps/web/messages/{ko,en}.json`, `apps/web/e2e/theme-picker.spec.ts`, `docs/design-system.md`, `AGENTS.md`(디자인 상태 갱신), `CHANGELOG.md`, plan `docs/superpowers/plans/2026-05-16-design-system-adoption.md` | 사용자 결정 11건(mockup 2회 검증 + 토론 5라운드): Q5=B(swap) · 5테마 확정 · Sunset 제외(hex 충돌) · Forest hex 보정 · color-mix 파생 · 사이드바 active 변경 · 다크 적용 · UserMenu submenu(option B) · status chip 형태 유지 · shadow-soft 유지 · border 0.10 유지. 단일 PR(worktree `design-system-adoption`). |
