...
- Drizzle 마이그레이션은 자체 baseline을 생성:
  ```bash
  pnpm --filter @jarvis/db generate --name baseline
  ```
...
- **`pnpm db:reset`은 외부 사용자도 안전**:
  ```bash
  DATABASE_URL=postgres://... pnpm db:reset
  ```
  drop schema → migrate → seed → ETL apply 까지 모두 실행하고, `packages/db/seed/dev.ts` + `.local/legacy-ssms/postgres/` 부재 시 해당 단계는 자동 skip.
...
- `packages/db/seed/` — TS 시드 스크립트 (workspace·role·permission·menu·holiday — Layer 1 bootstrap)
- `packages/db/drizzle/` — Drizzle 마이그레이션 SQL + `_journal.json` 무결성 해시. SQL을 절대 직접 수정하지 말 것 (해시 mismatch로 마이그레이션이 깨짐)
- `.local/legacy-ssms/postgres/<NN>_<module>/*.sql` — Layer 2 ETL 출력 SQL (legacy Oracle → jarvis 변환 결과). 멱등 INSERT, deterministic UUID
- `.local/scripts/db-reset.sh` — Layer 3 runner (`pnpm db:reset`이 호출). drop drizzle+public schema → migrate → seed conditional → ETL apply
- `.local/scripts/build-db-private-zip.mjs` — 본 zip을 다음 회차에 다시 빌드하는 도구 (사내 dev workstation 전용)
- `.local/docs/2026-05-04-legacy-etl-meta-strategy-{design,plan}.md` + `HANDOFF.md` — 메타 전략 spec/plan + 작업 인수인계 문서
...
복원 후 — **단일 명령으로 wipe-and-reload**:
```bash
pnpm install --frozen-lockfile
DATABASE_URL=postgres://... WORKSPACE_ID=<uuid> pnpm db:reset
```

`pnpm db:reset` 흐름 (per `.local/scripts/db-reset.sh` — 사내 zip 채널로 복원):
1. `DROP SCHEMA IF EXISTS drizzle CASCADE; DROP SCHEMA public CASCADE; CREATE SCHEMA public;`
2. `pnpm db:migrate` (drizzle migrations 적용)
3. `pnpm db:seed` (TS bootstrap — `packages/db/seed/dev.ts` 존재 시만)
4. `find .local/legacy-ssms/postgres -name '*.sql' | sort | xargs psql -v WORKSPACE_ID=…` (lex order)

ETL 작성 워크플로 + 폴더 컨벤션 (10/20/30 단위 prefix, 멱등 INSERT, deterministic UUID 등)은 사내 zip 채널의 `.local/docs/2026-05-04-legacy-etl-meta-strategy-design.md` §3-§4 참조.
