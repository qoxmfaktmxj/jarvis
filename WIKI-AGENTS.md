...
### 역할 × 위키 접근 (2026-05-12 단순화)

행 단위 sensitivity 필터는 폐기되었다. 위키 접근은 **RBAC 권한 + workspaceId 일치**로만 결정한다.

| 역할 | 접근 |
|------|------|
| VIEWER | `KNOWLEDGE_READ` 보유 → workspace의 위키 페이지 read 가능 |
| DEVELOPER | + `KNOWLEDGE_UPDATE`, `SYSTEM_ACCESS_SECRET` (Project 시크릿 reveal) |
| MANAGER | + `KNOWLEDGE_REVIEW`, `KNOWLEDGE_CREATE`, manual 영역 편집 |
| ADMIN | `ADMIN_ALL` — 모든 권한 |

> **2026-05-12 변경:** 8 테이블 sensitivity 컬럼 일괄 drop + 14개 헬퍼(`canAccessKnowledgeSensitivity`/`buildWikiSensitivitySqlFilter`/`canViewWikiPage`/`canViewInternalNotice`/`canResolveProjectSecrets` 등) 완전 제거.
> 격리는 RBAC 권한 + workspaceId 멀티테넌트 모델로 일원화 — 권한 없는 사용자는 화면 자체가 노출되지 않는다.
