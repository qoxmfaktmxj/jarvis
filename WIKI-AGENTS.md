...
[프로세스]
  ...
  Step C (Write):
    ...
    - validate (frontmatter 스키마, wikilink 존재)
  ...
  Step D (Review queue):
    - contradictions / PII 감지 → review_queue로
...
[프로세스]
  ...
  4. RBAC 권한 + workspaceId 필터 (DB WHERE) — 행 단위 sensitivity 필터는 2026-05-12 폐기
...
| 충돌 지점 | 해결 |
|-----------|------|
| 파일시스템엔 RBAC 없음 | 서버 미들웨어 게이트(`requirePermission` + workspaceId). 디스크는 **서버 내부 전용**, 사용자는 HTTP로만 접근. 행 단위 sensitivity 필터는 2026-05-12 폐기 |
...
### 역할 × 위키 접근 (2026-05-16 RBAC 단순화)

행 단위 sensitivity 필터는 폐기되었다(2026-05-12). 위키 접근은 **RBAC 권한 + workspaceId 일치 + owner check(knowledge·schedule 한정)**로만 결정한다.

| 역할 (code · 한글) | 접근 |
|------|------|
| MEMBER · 일반 | `KNOWLEDGE_READ` 보유 → workspace의 위키 페이지 read 가능. 수정·삭제 불가 |
| MANAGER · 매니저 | `KNOWLEDGE_READ` + `KNOWLEDGE_ADMIN` (manual 편집/리뷰 제출/삭제 가능). 단, owner check — 본인 작성(`created_by`) 또는 `knowledge_page_owner` row만 |
| ADMIN · 관리자 | `KNOWLEDGE_*` + `ADMIN_ALL` — owner check 우회, 모든 페이지 수정·삭제 가능 |
| YEAREND · 연말정산 | jarvis 권한 0개 — 위키 접근 불가 (외부 yearend 사이트 식별 라벨) |

> **2026-05-16 RBAC simplification:** 47 권한 · 5 역할 → 23 권한 · 4 역할. `KNOWLEDGE_CREATE/UPDATE/DELETE/REVIEW` 폐기 → `KNOWLEDGE_ADMIN`로 흡수. `SYSTEM_ACCESS_SECRET`/`PROJECT_ACCESS_SECRET` 폐기 → `PROJECT_READ` 보유 시 모두 노출(D5). 한글 라벨 `ROLE_LABELS` 신설. owner check 패턴은 knowledge·schedule 2도메인에만 적용 (notice/project/maintenance/sales는 권한 부여로 통제).
> **2026-05-12 변경:** 8 테이블 sensitivity 컬럼 일괄 drop + 14개 헬퍼 완전 제거. 격리는 RBAC 권한 + workspaceId 멀티테넌트 모델로 일원화 — 권한 없는 사용자는 화면 자체가 노출되지 않는다.
...
```
wiki_page_index         # 파일 시스템 미러: path, title, frontmatter, workspaceId, updatedAt, gitSha (sensitivity/required_permission 컬럼은 2026-05-12 drop)
...
wiki_review_queue       # contradictions, PII 감지 등 수동 승인 대기
...
Jarvis 하네스(`jarvis-feature` 진입점 + superpowers 방법론 + 도메인 스킬 4개, 2026-04-22 재편)에서 호출 가능. Python CLI는 포팅하지 않음(Node-only).
