...
### Sensitivity × 역할 접근 매트릭스 (Phase-W3 기준)

`buildWikiSensitivitySqlFilter` (`packages/auth/rbac.ts`) 기준 wiki 검색·AI 결과 가시성.

| sensitivity | VIEWER | DEVELOPER | MANAGER | ADMIN |
|-------------|--------|-----------|---------|-------|
| PUBLIC | ✅ | ✅ | ✅ | ✅ |
| INTERNAL | ✅ | ✅ | ✅ | ✅ |
| RESTRICTED | ❌ | ❌ | ✅ (`KNOWLEDGE_REVIEW`) | ✅ |
| SECRET_REF_ONLY | ❌ | ✅ (`SYSTEM_ACCESS_SECRET`) | ❌ | ✅ |

> **Phase-W3 변경 (2026-04-15):** `DEVELOPER`의 `RESTRICTED` 접근이 제거됨.
> 기존 SQL 필터(`buildKnowledgeSensitivitySqlFilter`)는 `KNOWLEDGE_UPDATE` 보유 시 RESTRICTED를 허용했으나,
> 신규 헬퍼는 `wiki-sensitivity.ts`의 본문 열람 게이트와 규약을 통일.
> DEVELOPER가 RESTRICTED 위키 접근이 필요하면 MANAGER 권한 추가 부여 필요.
