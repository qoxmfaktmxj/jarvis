...
export function StalePagesWidget({ pages }: { pages: StalePage[] }) {
  ...
  return (
    <section aria-label={t("title")} className="flex h-full flex-col">
      ...
      {pages.length === 0 ? (
        <p className="text-sm text-surface-500">{t("allFresh")}</p>
      ) : (
            ...
            <li key={page.id} className="py-2.5 first:pt-0 last:pb-0">
              <div className="flex items-start justify-between gap-3">
                <Link
                  ...
                  className="line-clamp-2 min-w-0 flex-1 text-sm font-medium text-surface-800 transition-colors duration-150 hover:text-isu-600"
                >
                  {page.title}
                </Link>
