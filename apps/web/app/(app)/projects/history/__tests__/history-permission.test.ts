...
describe("project history actions guards", () => {
  ...
  it("filters mutations and reads by workspaceId", () => {
    expect(source).toContain("eq(projectHistory.workspaceId, ctx.workspaceId)");
  ...
  it("uses transaction, audit log, and Zod output parsing", () => {
    ...
    expect(source).toContain("listProjectHistoryOutput.parse");
    expect(source).toContain("saveProjectHistoryOutput.parse");
  });
});
