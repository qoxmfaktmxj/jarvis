...
export async function listSalesCompanies(rawInput: unknown) {
...
export async function saveSalesCompanies(rawInput: unknown) {
  const ctx = await resolveSalesContext();
  if (!ctx.ok) {
    ...
    await db.transaction(async (tx) => {
      ...
      for (const update of input.updates) {
        ...
        const [row] = await tx
          ...
          .where(and(eq(company.id, id), eq(company.workspaceId, ctx.workspaceId)))
          .returning({ id: company.id });
