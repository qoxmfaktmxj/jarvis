...
const ENV_STYLES: Record<string, string> = {
  ...
  dev: 'bg-isu-50 text-isu-700 ring-isu-500/25',
  local: 'bg-surface-100 text-surface-700 ring-surface-300',
};
...
export function InfraRunbookHeader({ meta }: InfraRunbookHeaderProps) {
  ...
  const envChip = ENV_STYLES[envKey] ?? 'bg-surface-100 text-surface-700 ring-surface-300';
  ...
  return (
    ...
    >
      {/* Header bar */}
      <div className="flex items-center gap-2 border-b border-surface-200 bg-surface-50/70 px-4 py-2.5">
        ...
        <span className="text-display text-[10px] font-semibold uppercase tracking-[0.14em] text-isu-700">
          Infra runbook
        </span>
        <span className="ml-auto flex flex-wrap items-center gap-1.5">
          {meta.companyCd && (
            <span className="inline-flex items-center rounded-full bg-surface-100 px-2 py-0.5 text-[10px] font-semibold text-surface-600 ring-1 ring-inset ring-surface-200">
      ...
      {/* Footer hint */}
      <div className="flex items-center gap-1.5 border-t border-surface-100 bg-surface-50/50 px-4 py-2 text-[11px] text-surface-500">
        <Terminal className="h-3 w-3 text-surface-400" />
...
function MetaRow({ row, isLast }: { row: Row; isLast: boolean }) {
  return (
    <div
      className={cn(
        'grid grid-cols-[auto_1fr] items-start gap-x-3 py-2 sm:border-b sm:border-surface-100',
        isLast && 'sm:border-b-0',
      )}
    >
      <dt className="text-display w-[110px] shrink-0 pt-0.5 text-[11px] font-medium uppercase tracking-wide text-surface-500">
        {row.label}
      </dt>
      <dd className="group flex min-w-0 items-start gap-1.5">
        <span
          className={cn(
            'min-w-0 break-all text-[13px] text-surface-800',
        ...
        {row.mono && (
          <button
            ...
            className="ml-auto mt-0.5 shrink-0 rounded p-0.5 text-surface-300 opacity-0 transition-all hover:bg-surface-100 hover:text-isu-600 group-hover:opacity-100"
