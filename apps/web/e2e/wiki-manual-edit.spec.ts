...
test.describe('Wiki Manual Edit — Tiptap Editor', () => {
  ...
  test.skip('manual 편집 페이지 접근 — /wiki/manual/[workspaceId]/[path]/edit', async ({ page }) => {
    // TODO: Phase-W2 완료 후 활성화
    // 에디터 컴포넌트 렌더 확인
    await page.goto('/wiki/manual/default/test-page/edit');
  ...
  test.skip('auto/ 경로에 manual 편집 UI 없음 — 라우팅 분리 확인', async ({ page }) => {
    ...
    const response = await page.goto('/wiki/auto/default/sample/edit');
