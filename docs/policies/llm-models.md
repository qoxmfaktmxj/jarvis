...
| 모델 | 차원 | 비고 |
|------|------|------|
| `text-embedding-3-small` | 1536 | `knowledge_page.embedding` 컬럼 + HNSW cosine 인덱스와 **계약된 고정 값**. 변경 시 마이그레이션 + 전량 재임베딩 필요 → 실질적으로 고정 |
...
- 임베딩: `text-embedding-ada-002`, `text-embedding-3-large`
