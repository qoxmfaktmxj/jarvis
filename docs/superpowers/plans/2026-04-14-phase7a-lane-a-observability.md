...
- `packages/db/drizzle/<timestamp>_llm_call_log.sql` — `pnpm db:generate`가 생성.
...
- Generate: `packages/db/drizzle/*_llm_call_log.sql`
...
Expected: `packages/db/drizzle/` 아래 `NNNN_*_llm_call_log.sql` 생성. 파일 내 `CREATE TABLE "llm_call_log"` + 3개 `CREATE INDEX` 포함.
...
grep -l "llm_call_log" packages/db/drizzle/*.sql
```
...
```bash
git add packages/db/schema/llm-call-log.ts packages/db/schema/index.ts packages/db/drizzle/
...
## Task 5.1: Sentry 초기화 진입점 배선

Spec §3 PR#1 "각 진입점 초기화" 요구사항. wrapper만 만들고 호출 안 하면 무용지물이므로 web/worker 진입점에서 실제로 `initSentry()`를 호출한다.

**Files:**
- Create: `apps/web/instrumentation.ts` (Next.js 15 표준 진입점)
- Modify: `apps/worker/src/index.ts` (상단에 init 호출 추가)
- Create: `packages/shared/__tests__/sentry.init.test.ts` (DSN 있을 때 Sentry.init 호출, 없을 때 no-op 검증)

- [ ] **Step 1: 실패 테스트 작성**

```ts
// packages/shared/__tests__/sentry.init.test.ts
import { describe, it, expect, vi, beforeEach } from "vitest";
...
```bash
pnpm --filter @jarvis/shared test -- sentry.init
```

Expected: 진입점 파일이 없는 상태에서도 wrapper 자체는 동작할 수 있으므로 단위 테스트는 pass할 수 있으나, 앱 부팅 시 실제로는 init이 실행되지 않음 — 다음 스텝에서 web/worker 진입점을 추가해 배선한다.

- [ ] **Step 2: `apps/web/instrumentation.ts` 생성**
...
- [ ] **Step 3: `apps/worker/src/index.ts` 상단에 init 호출 추가**

기존 import 블록 맨 위에 아래 2줄을 삽입한다(파일 전체 교체 아님, 최소 패치):
...
Expected: sentry.init 테스트 pass, web/worker typecheck 성공(새 import 경로 해결 확인).

- [ ] **Step 5: 커밋**

```bash
git add apps/web/instrumentation.ts apps/worker/src/index.ts packages/shared/__tests__/sentry.init.test.ts
git commit -m "feat(observability): Sentry init을 web/worker 진입점에 배선"
```
...
> **주의**: `embed.ts` 기존 로직(OpenAI 호출, 에러 처리)을 유지하면서 logger 호출만 추가한다. 파일 전체 교체가 아니라 `openai.embeddings.create()` 호출 전후에 log 코드를 삽입하는 patch 방식. 아래 예시 코드는 최종 형태 참고용이며, 실제 작업은 기존 파일에 `logLlmCall` + `assertBudget` import와 호출부만 삽입한다.
...
`packages/ai/embed.ts`에 아래 형태로 `logLlmCall` + `assertBudget` 삽입(기존 OpenAI 호출/에러 처리 로직은 유지, patch 방식). 최종 형태 참고용 전체 예시:
...
**Files:**
- Create: `apps/web/app/admin/llm-cost/__tests__/page.test.tsx`
- Create: `apps/web/app/admin/llm-cost/page.tsx`

> **플랜 주의**: 아래 smoke test는 `@testing-library/react` + vitest 조합을 전제한다. 프로젝트가 해당 라이브러리를 아직 포함하지 않는 경우, 동일한 의도(페이지가 "LLM 비용" 또는 "LLM Cost" heading을 포함하는지)를 확인하는 형태로 테스트 형태를 조정한다(예: Server Component를 직접 호출해 반환 React element를 `renderToString`으로 검사). 구현부는 그대로 두고 test helper만 조정할 것.

- [ ] **Step 1: 실패하는 smoke test 작성**

```tsx
// apps/web/app/admin/llm-cost/__tests__/page.test.tsx
...
describe('admin llm-cost page', () => {
  it('renders cost dashboard with heading', async () => {
    ...
    expect(container.textContent?.toLowerCase()).toMatch(/cost|비용/);
  });
});
```

```bash
pnpm --filter web test -- llm-cost
```

Expected: 모듈 resolve 실패("Cannot find module '../page'" 또는 유사) — 다음 스텝에서 구현한다.
...
Expected: smoke test pass, 빌드 성공, 새 라우트 `/admin/llm-cost` 출력.

- [ ] **Step 4: 커밋**

```bash
git add apps/web/app/admin/llm-cost/page.tsx apps/web/app/admin/llm-cost/__tests__/page.test.tsx
...
출력된 URL은 PR 본문/작업 노트에만 남긴다. Spec §9 Revision log는 PR#G에서 일괄 반영하므로 **Lane A PR은 `docs/superpowers/specs/2026-04-14-phase7-v3-design.md` 파일을 건드리지 않는다.**
