...
- **변경**:
  - `packages/db/src/schema/llm-call-log.ts` (신규)
  - `packages/ai/src/logger.ts` (pino + request-id 컨텍스트)
  - `apps/web/src/middleware.ts` (request-id 주입)
  - Sentry 연동 (위치: `packages/observability/` 또는 기존)
...
- **변경**:
  - `packages/ai/src/budget.ts` (신규): `LLM_DAILY_BUDGET_USD` 체크
  - OpenAI 호출 진입점에 `assertBudget()` 게이트
  - 비용 대시보드: 쿼리 뷰 또는 `/admin/llm-cost` 페이지
  - `pnpm eval:budget-test` 스크립트 (G1에서 사용할 시뮬레이션 harness)
...
- **변경**:
  - `packages/ingest/src/pii-redactor.ts` (신규): 주민번호/전화/이메일/카드 패턴
  - `packages/db/src/schema/review-queue.ts` (신규)
  - ingest pipeline 최상단 Step 0 삽입
  - 규칙: PII 탐지 시 `sensitivity` 최소 `INTERNAL`, SECRET 키워드 매치 시 `review_queue` enqueue
- **완료 조건**: unit 100% pass + integration (`apps/worker/test/integration/pii-flow.test.ts`) 1건 — PII 문서 → redact → sensitivity 승급 → review_queue 1행
...
- **변경**: `packages/ai/src/cache.ts` (또는 기존 캐시 위치)
...
- **변경**:
  - `packages/db/src/schema/document-chunks.ts` (신규, customType vector 1536d 사용)
  - migration 파일
...
- **산출물**:
  - `apps/worker/test/integration/cross-workspace-leakage.test.ts` (신규)
  - CI 워크플로우에서 `pnpm test:integration` 포함
...
| G2 | PII redactor | vitest unit (`packages/ingest/test/pii-redactor.test.ts`) | 주민번호·전화·이메일·카드 각 5건 | 100% pass | CI artifact |
| G3 | review_queue | vitest integration (`apps/worker/test/integration/pii-flow.test.ts`) | SECRET 키워드 문서 1건 | `review_queue` 1행 + sensitivity 승급 | CI artifact |
| G4 | cross-workspace leakage | vitest integration (`apps/worker/test/integration/cross-workspace-leakage.test.ts`) | workspace A/B seed, 쿼리 3종 | B chunk 0건 in top-50 | CI artifact |
