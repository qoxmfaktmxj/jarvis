 ...
 *
 * Sample data so charts render realistically. 24 months × 5 depts × 3 gubun × 3 metric ≈ 1080 rows.
...
// Margin ratios per metric vs SALES baseline.
...
// Gubun adjustments — actual diverges from plan, forecast trends slightly above plan.
...
function deterministicNoise(ym: string, dept: string, gubun: string, metric: string): number {
  // Simple hash → -0.15..+0.15 jitter
  const s = `${ym}|${dept}|${gubun}|${metric}`;
  let h = 0;
  for (const ch of s) h = (h * 31 + ch.charCodeAt(0)) | 0;
  const norm = ((h >>> 0) % 30000) / 100000; // 0..0.30
  return 1 + (norm - 0.15);
}

function* months(start: string, end: string): Generator<string> {
  const [sy, sm] = [Number(start.slice(0, 4)), Number(start.slice(4, 6))];
  const [ey, em] = [Number(end.slice(0, 4)), Number(end.slice(4, 6))];
  let y = sy, m = sm;
...
export async function seedSalesPlanPerf(workspaceId: string): Promise<void> {
  ...
  // Drizzle batch limit safety — chunk by 500
  const CHUNK = 500;
