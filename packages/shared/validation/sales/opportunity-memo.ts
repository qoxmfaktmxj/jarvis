...
// 입출력
export const opportunityMemoListInput = z.object({ opportunityId: z.string().uuid() });
export const opportunityMemoListOutput = z.object({
  rows: z.array(memoTreeNodeSchema),
});

export const opportunityMemoCreateInput = z.object({
  opportunityId: z.string().uuid(),
  priorComtSeq: z.number().int().min(0), // 0 = 마스터 의견, >0 = reply
  memo: z.string().min(1).max(4000),
});
export const opportunityMemoCreateOutput = z.object({
  ok: z.boolean(),
  comtSeq: z.number().int().nullable(),
});

export const opportunityMemoDeleteInput = z.object({
  opportunityId: z.string().uuid(),
  comtSeq: z.number().int(),
});
export const opportunityMemoDeleteOutput = z.object({ ok: z.boolean() });
