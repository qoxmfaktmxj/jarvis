---
title: "납부지연 가산세 일 0.022%"
slug: withholding-employee-local-tax-penalty-late-payment-f-lt0017
pageType: guide
publishedStatus: published
sources:
  - sourceRevisionId: "{{sourceRevisionId:withholding-tax-verified-facts.json}}"
    locator: fact-f-lt0017
    effectiveDate: 2026-06-12
    confidence: 0.9
aliases: ["지방세기본법 제55조"]
tags: [withholding-tax, employee-local-tax, high]
created: 2026-06-12T00:00:00.000Z
updated: 2026-06-12T00:00:00.000Z
freshnessSlaDays: 90
---
# 납부지연 가산세 일 0.022%

종업원분 주민세 미납 시 미납세액 × 1일 0.022%(연 약 8.03%)의 납부지연 가산세가 부과된다(지방세기본법 제55조).

## 근거와 적용 범위

- 법적 근거: 지방세기본법 제55조
- 공식 출처: [원문 확인](https://www.law.go.kr/법령/지방세기본법)
- 자료 기준일: 2026-06-12
- 적용 기준일: 2026-06-12
- 다음 검토일: 2027-06-30
- 적용 범위·주의: 가산세 감면·중복배제·부정행위 해당성은 신고 경위와 지방세기본법 세부 규정 확인 필요.

## 검증 이력

- 원천 데이터: [withhold-tax `fa056fec22cc38db8b40c0d7ab3c1f2b54746e37`](https://github.com/qoxmfaktmxj/withhold-tax/blob/fa056fec22cc38db8b40c0d7ab3c1f2b54746e37/content/facts.json)
- 포함 기준: 확정, 1차 출처 검증 완료, 정부·공공기관 공식 URL

> 이 문서는 업무 참고용이며 법률·세무 자문이 아닙니다. 실제 처리 전 최신 법령과 과세관청 안내를 다시 확인하세요.
